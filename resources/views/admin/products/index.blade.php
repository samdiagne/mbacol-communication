                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium sticky right-0 bg-white">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            <button type="button"
                                    x-on:click="$dispatch('confirm-delete', { action: '{{ route('admin.products.destroy', $product) }}', message: 'Êtes-vous sûr de vouloir supprimer le produit « {{ addslashes($product->name) }} » ? Cette action est irréversible.' })"
...
{{-- Modal de partage après création de produit --}}
@if(session('share_product'))
@php $shareProduct = session('share_product'); @endphp
<div x-data="{ open: true, copied: false }" x-show="open" x-cloak
     class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
    <div class="flex items-center justify-center min-h-screen px-4 py-8">
        <div class="fixed inset-0 bg-gray-900/75" @click="open = false"></div>
        <div class="bg-white rounded-2xl shadow-2xl p-6 sm:p-8 max-w-lg w-full relative z-10" x-transition>
            ...
            <div class="flex items-center justify-between mb-5">
                <div class="flex items-center gap-3">
                    ...
                    <h3 class="text-lg font-bold text-gray-900">Produit ajouté !</h3>
                </div>
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden mb-6 shadow-sm">
                ...
                @else
                <div class="aspect-[1.91/1] bg-gray-100 overflow-hidden">
                    <img src="{{ asset('images/og-default.jpg') }}" alt="Mbacol Communication"
                         class="w-full h-full object-cover">
                </div>
                @endif
                <div class="p-4 bg-gray-50">
                    <p class="text-xs text-gray-500 uppercase tracking-wide mb-1">mbacol313.com</p>
                    <p class="font-semibold text-gray-900 text-sm leading-snug">{{ $shareProduct['name'] }} | {{ $shareProduct['price'] }}</p>
                    @if($shareProduct['description'])
                    <p class="text-xs text-gray-500 mt-1 line-clamp-2">{{ $shareProduct['description'] }}</p>
                    @endif
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 mb-6">
                <!-- WhatsApp -->
                <a href="https://wa.me/?text={{ urlencode($shareProduct['url']) }}"
                   target="_blank" rel="noopener noreferrer"
                   class="flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white text-sm font-medium px-4 py-2.5 rounded-xl transition">
                ...
                <!-- Facebook -->
                <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode($shareProduct['url']) }}"
                   target="_blank" rel="noopener noreferrer"
                ...
                <!-- Copier lien -->
                <button @click="navigator.clipboard.writeText('{{ $shareProduct['url'] }}'); copied = true; setTimeout(() => copied = false, 2000)"
                        class="flex items-center gap-2 text-sm font-medium px-4 py-2.5 rounded-xl transition"
                        :class="copied ? 'bg-green-100 text-green-700' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'">
